#!/usr/bin/env python3
"""
Cafe Music Content Generator - ChatGPT Version
카페 음악 콘텐츠 자동 생성 시스템 - ChatGPT 버전 (GPT-4o-mini)
"""
...
카페에서 편안하게 들을 수 있고, 집중력을 높이거나 휴식을 취하기 좋은 음악이 되도록 해주세요."""
        ...
        response = self.client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "You are a professional cafe music expert who generates metadata in JSON format."},
...
유튜브 SEO를 고려하여 검색이 잘 되도록 작성해주세요."""
        ...
        response = self.client.chat.completions.create(
            model=self.config['openai']['model'],
            messages=[
                {"role": "system", "content": "You are a YouTube SEO expert who generates metadata in JSON format."},
                {"role": "user", "content": prompt}
            ],
            temperature=self.config['openai']['temperature'],
            max_tokens=self.config['openai']['max_tokens'],
...
def main():
    ...
    parser = argparse.ArgumentParser(description='카페 음악 콘텐츠 자동 생성 (ChatGPT 버전)')
    ...
    print("🎵 카페 음악 콘텐츠 생성기 시작 (ChatGPT 버전)")
    print(f"   모델: GPT-4o-mini (저렴하고 빠른 버전)")
